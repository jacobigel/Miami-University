...
The data that we have is Miami University Admissions Data from 2009 - 2017. Through cleaning the data and eventually using models, we want to predict the probability of students leaving the university to incoming all freshman (probability of not being retained). In the end, we want to find the most important variables for Admissions to look for in a student to put more attention towards retaining them. 


### Reading in Data
```{r}
pacman::p_load(tidyverse, doParallel, nnet, caret, ranger, ROCR, gbm, xgboost, pROC, fastDummies, NeuralNetTools)
...
lrstep<-readRDS("lrstep1.RDS")
step2 <-readRDS("f.RDS")
nnetFit<-readRDS("nnet_final.RDS")
...
domestic <-select(domestic, -"MiamiRanks_9", -"MiamiRanks_10", -"MiamiRanks_S",
                  -"HighSchoolState_AR", -"HighSchoolState_DE",
                  -"HighSchoolState_OR",-"MiamiRanks_8",
                  ...
                  -"Division_CCA", -"AlumniConnection_M - Mother", 
                  -"AlumniConnection_F - Father",
...
domestic <-select(domestic, -"SuppMajor3_AP56", -"SuppMajor3_AS47", -"SuppMajor3_AS86", 
                  -"SuppMajor3_ASBC", -"SuppMajor3_ASBP" , -"SuppMajor3_ASEV",
                  -"SuppMajor3_ASIN", 
                  -"SuppMajor3_ASJN", -"SuppMajor3_ASSW", -"SuppMajor3_ASU1",
                  -"SuppMajor3_BU01", 
                  -"SuppMajor3_BUIB", -"SuppMajor3_BUIS", -"SuppMajor3_EA56",
                  -"SuppMajor3_EAGA", 
...
f<-formula(retained ~ (ConfirmCodeY + ClassSize + GPA + ACTComposite + ACTEng + 
                         ...
                         AlmostFullRaceWhite + AlumniConnection_O + `\`AlumniConnection_O -
                         Other\`` + 
                         AlumniConnection_S + AlumniConnection_NA + Division_AP + 
                         Division_BU + Division_EA + Division_FSB + ApplicationType_OF + 
                         ApplicationType_OM + DecisionType_H + DecisionType_K +
                         DecisionType_NA + 
...
AUC Difference: 0.288

I think the AUC values are a little too far off to be considered good enough for this data set. We have other models that we ran that have less of a difference.
...
We still think that this is a little too wide of a difference for us to consider this the final model used. 
```{r}
gb.tree
plot(gb.tree)
...
#### ROC Objects
```{r, eval = FALSE}
results<-data.frame(valid$retained, p.gbtree[,2], p.xgboost[,2])
```

#### Lift
```{r, eval = FALSE}
pred6<-prediction(results$p.xgboost, results$valid.retained)
lift6<-performance(pred6, "lift", "rpp")
plot(lift6,main="Lift chart", col="brown1", ylim=c(0,2))

varImp(xgboost)

confusionMatrix(data=results$Cut, reference=results$valid.retained, positive="No")

```

``` {r, eval = FALSE}
pred<-prediction(results$ps, results$valid.retained)
pred2<-prediction(results$psit, results$valid.retained)
pred3<-prediction(results$p.rforest, results$valid.retained)
pred4<-prediction(results$p.gbtree, results$valid.retained)
pred5<-prediction(results$p.nnet, results$valid.retained)
pred6<-prediction(results$p.xgboost, results$valid.retained)
...
plot(lift6, col="black", add=T)
legend(0.6, 1, legend=c("First Order Log", "Second Order Log", "Random Forest", "Boosted Tree", "NN1", "xgboost"),
       col=c("brown1", "blue", "chocolate2", "chartreuse", "darkmagenta", "black"), lty=1, cex=0.8)
```
...
plot(lift5, col="darkmagenta", add=T)
plot(lift6, col="black", add=T)
legend(0.05, 1, legend=c("First Order Log", "Second Order Log", "Random Forest", "Boosted Tree", "NN1", "xgboost"),
       col=c("brown1", "blue", "chocolate2", "chartreuse", "darkmagenta", "black"), lty=1, cex=0.8)
```
...
```{r, eval = FALSE}
roc<-list( "Step"=ps, "Sec.Order"=psit, "Random Forest"=r.rforest, "Small Tree"=r.small.valid, "GB Tree"=r.gbtree, "XGBoost"=r.xgbtree)
